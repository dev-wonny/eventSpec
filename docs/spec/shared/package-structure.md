...
- 이벤트 타입 확장은 JPA 상속보다 `EventEntity + DomainService` 전략을 우선한다.
- `application.service`는 유스케이스 흐름 조립 계층으로 사용한다.
- `domain.service`는 도메인 규칙 및 비즈니스 규칙 판단 계층으로 사용한다.
...
- Domain Service: 도메인 규칙 및 비즈니스 규칙 판단
...
현재 출석 API는 Request Body가 없으므로 request DTO는 당장 필수가 아니다. 헤더는 interceptor + argument resolver로 주입하고, validation 메시지는 전역 예외 처리기 기준으로 맞춘다.
...
현재 출석체크 구현에서는 `AttendEventTransactionService`가 `entry -> win`까지 로컬 저장을 끝내고 `PointGrantCommand`를 event로 발행한 뒤, `AFTER_COMMIT` listener가 외부 reward 호출을 맡는 구조를 권장한다.
...
외부 point API처럼 로컬 트랜잭션 commit 이후에만 실행되어야 하는 후처리는 이 계층에 둔다.
...
이벤트 타입별 차이를 `AttendanceEvent extends Event` 같은 상속 구조로 풀기보다, 공통 모델 + `DomainService` 조합으로 해석하는 방식을 기본으로 한다.
...
이 계층에는 이벤트 타입별 규칙과 공통 "규칙 계산"을 둔다. HTTP, JPA 조회 조합, 외부 API 호출, transaction 경계는 두지 않는다.
Repository를 직접 호출하지 않고, `Application Service`가 준비한 데이터를 받아 판단만 수행한다.

별도 정책 패키지는 기본 구조로 두지 않고, 타입별 규칙은 `...DomainService` 안에 녹여 관리하는 것을 권장한다.
...
- `PointApiFeignClient`
- `PointApiClientConfig`
- `PointGrantRequest`
- `PointGrantResponse`

외부 point API 스키마는 여기서 격리한다. 현재는 Spring Cloud 없이 pure OpenFeign 기반 클라이언트를 권장한다.
...
요청 식별과 트래픽 로그 필터는 모두 이 패키지에 둔다.
로그 key 상수는 공용 `LogContextKeys` 클래스로 분리하지 않고 각 필터/클래스 상단에 선언하는 것을 권장한다.
...
- Java 코드의 null 체크는 동등 연산자 기반 비교 대신 `Objects.isNull(value)`, `Objects.nonNull(value)`를 사용한다.
- 조건문, 삼항 연산자, 문서의 코드 예시에도 같은 규칙을 적용한다.

## 상수 사용 규칙

- Java 코드에서 의미 있는 기본값은 숫자 리터럴로 직접 쓰지 않고 `private static final` 상수로 선언한다.
- 예를 들어 weight 기본값은 `1`을 직접 넣지 않고 `private static final Integer DEFAULT_WEIGHT = 1;` 형태로 관리한다.
- DB DDL의 `DEFAULT 1` 같은 스키마 기본값은 유지할 수 있지만, 애플리케이션 코드에서는 같은 의미를 상수 이름으로 드러내야 한다.
...
- 공통 로그 필드명은 필요 시 `common.logging`에 둘 수 있지만, 필터 구현은 `infrastructure.filter`에 둔다.
...
7. 로그 관련 필터는 `infrastructure.filter`에 둔다.
