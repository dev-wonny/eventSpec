...
이벤트는 하나의 DB 테이블(`event`)에 저장되지만, `event_type`에 따라 서로 다른 도메인 동작과 규칙을 가진다. 따라서 아래 목표를 가진 구조를 설계한다.

- Event 테이블은 하나로 유지
- `EventType`에 따라 도메인 규칙 분리
- 이벤트별 로직 분리
- 확장 가능한 이벤트 구조
- JPA / QueryDSL 기반 persistence 유지

## 2. JPA 상속 대신 DomainService 분리 전략 채택

이벤트 플랫폼은 `JPA 상속`보다 `DomainService` 분리 방식을 우선 선택한다.
...
- 현재 플랫폼은 단일 `event` 테이블을 유지해야 하므로, 타입별 행위를 Entity 상속 계층에 담는 방식보다 `EventEntity + DomainService` 조합이 더 단순하고 확장에 유리하다.
- 현재 구현 기준으로는 별도 정책 계층을 두기보다, 이벤트 타입별 규칙을 `AttendanceDomainService` 같은 `DomainService` 안에 녹여 관리한다.

권장 구조:

```text
EventEntity (data)
DomainService (behavior)
...
- `DomainService`: 이벤트 타입별 행위와 도메인 규칙 판단
...
- 이벤트 타입별 확장의 기본 수단은 `JPA 상속`이 아니라 `DomainService` 구현 분리다.
- 타입별 보조 모델이 필요하면 조회/계산 보조 객체로 둘 수 있지만, `EventEntity`의 상속 구조로 풀지는 않는다.

즉, DB 테이블은 하나지만 도메인 해석과 규칙은 `EventType`에 따라 달라질 수 있다.

## 8. 이벤트 타입별 DomainService

별도 정책 인터페이스를 두기보다, 이벤트 타입별 규칙은 `domain.service`의 `...DomainService`에 둔다.
...
이벤트 타입이 늘어나면 별도 정책 팩토리보다 타입별 `DomainService`를 추가하는 방향을 우선한다.
...
- `LOTTERY` -> `LotteryDomainService`

`Application Service`는 유스케이스에 맞는 `DomainService`를 직접 호출하거나, 실제로 다형성이 필요해지는 시점에만 선택 로직을 추가한다.

## 11. Domain Service 역할

`Domain Service`는 이벤트 타입별 규칙과 여러 Entity에 걸친 비즈니스 규칙 판단을 담당한다.
...
즉, 응모 규칙은 `DomainService`가 담당하고, 응모 유스케이스 실행은 `Application Service`가 담당한다.

현재 구현 범위는 출석체크 하나이므로, `entry / draw / reward`를 공통 인터페이스로 먼저 잘게 분리하기보다 `AttendanceProcessor` 같은 출석체크 전용 처리 컴포넌트로 묶는 쪽을 권장한다.
공통 추상화가 실제로 필요해지는 시점 전까지는 `AttendanceDomainService` 같은 타입별 `DomainService`를 중심으로 유지한다.

조회 유스케이스에서도 같은 원칙을 따른다.

- `Application Service`가 `Event`, `EventRound`, `Prize`, 확률 정보 같은 조회 데이터를 모은다.
...
- Entity에 도메인 규칙을 넣지 않는다.
- 도메인 규칙은 `DomainService`에서 처리한다.
- 이벤트 타입별 행위 차이는 JPA 상속이 아니라 `DomainService` 구현 분리로 해결한다.
...
2. `EventType`에 따라 `DomainService`를 분리한다.
...
7. 이벤트 타입 확장은 JPA 상속보다 `EventEntity + DomainService` 전략을 우선한다.
...
1. `EventType` 추가
2. 해당 이벤트 전용 `DomainService` 추가
3. 해당 유스케이스의 `Application Service`에서 호출 경로 연결
