...
- 기본 식별 기준은 `(event_id, round_id, member_id)`이며, 같은 회차에는 한 건만 존재해야 한다.
...
- 현재 프로젝트는 `event_applicant` 관리 API를 제공하지 않으므로 applicant 생성/적재는 SQL 또는 별도 admin 프로젝트에서 관리할 수 있다.

### ATT-RULE-004 실제 응모권 기록은 `event_entry`다

- 참여 이력의 SoT는 `event_entry`다.
- `event_entry`는 응모권/참여 이력 테이블이며 같은 회차에도 여러 건이 들어갈 수 있다.
- 출석 이벤트에서 `event_entry`는 회차별 출석 여부를 판정할 수 있도록 `event_id`, `round_id`를 가진다.
...
- 동일 사용자의 출석 중복 체크 비즈니스 기준은 `event_applicant`의 `event_id + round_id + member_id`다.
...
- 최소 unique 대상은 `event_round (event_id, round_no)`, `event_applicant (event_id, round_id, member_id)`, `event_win (entry_id)`다.
- 출석 중복은 `event_applicant`의 같은 키 조회와 검증으로 먼저 제어해야 한다.
- 동시 요청에서는 `uq_event_applicant_event_round_member` unique 충돌도 중복 출석으로 변환해야 한다.
...
- 보상 매핑이 있는 출석은 `event_applicant` 저장 후 `event_entry.is_winner = true`로 저장하고 외부 point API를 호출한다.
- 외부 point API 호출이 성공하면 `event_win`을 저장하고 최종 커밋한다.
- 외부 point API 호출이 실패하면 `event_entry`와 `event_win`은 모두 롤백되어야 한다.
- 외부 point API 호출이 실패한 요청은 출석 성공으로 간주하지 않으며 `event_applicant`도 함께 롤백되어야 한다.
...
- 출석 spec 기준으로는 `event_applicant`에 `(event_id, round_id, member_id)` 최소 unique와 `event_entry`에 `event_id`, `round_id`, `member_id` 기반 조회 경로가 필요하다.
- 신규 환경용 schema draft에는 FK 없이 `uq_event_round_event_round_no`, `uq_event_applicant_event_round_member`, `uq_event_win_entry_id`만 최소 unique로 반영한다.
