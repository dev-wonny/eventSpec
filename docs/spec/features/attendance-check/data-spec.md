...
4. `event_applicant`를 `event_id + round_id + member_id` 기준 회차별 applicant 레코드로 생성한다.
...
| ATT-DATA-010 | 보상 결과 이력 | `event.event_win` | `id`, `entry_id`, `round_id`, `event_id`, `member_id`, `event_round_prize_id`, `is_deleted` | ATT-REQ-002, ATT-REQ-004, ATT-REQ-005, ATT-REQ-006 | DDL 반영 | 로컬 당첨/보상 확정 시 생성 |
