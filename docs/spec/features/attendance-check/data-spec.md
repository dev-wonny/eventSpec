...
3. `event_applicant`에서 해당 사용자의 eligibility를 조회하고, `round_id`가 있으면 요청 회차와도 맞는지 확인한다.
4. 출석 회차에 연결된 단일 point 보상 매핑이 있는지 확인한다.
5. 보상 매핑이 있으면 외부 point API 호출 성공 후 `event_entry`, `event_win`을 함께 커밋한다.
6. 보상 매핑이 없으면 `event_entry`만 커밋한다.
...
| ATT-DATA-003 | 참여 가능 대상자 풀 | `event_platform.event_applicant` | `id`, `event_id`, `round_id`, `member_id`, `is_deleted` | ATT-REQ-002, ATT-REQ-005 | 업무 확정, schema draft 반영 | `(event_id, member_id)` unique, `round_id`는 선택값 |
...
| ATT-DATA-006 | applicant 고유성 요구사항 | 출석 spec 기준 | `(event_id, member_id)` | ATT-REQ-002 | 업무 확정, schema draft 반영 | 제공된 원본 DDL unique와 불일치 |
| ATT-DATA-007 | 출석 중복 판정 기준 | 출석 spec 기준 | `(event_id, round_id, member_id)` | ATT-REQ-003 | 업무 확정 | 비즈니스 기준으로 중복 출석을 판정 |
...
- 스키마 리스크: 제공된 원본 DDL은 업무상 확정된 `event_entry.event_id`, `event_entry.round_id`, `(event_id, member_id)`와 nullable `event_applicant.round_id`를 직접 지원하지 않았고, 신규 환경용 schema draft에서 이를 정합화했다.
