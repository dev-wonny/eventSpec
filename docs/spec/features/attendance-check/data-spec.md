...
- soft delete된 `event_applicant`는 현재 유효 eligibility로 보지 않으며, 동일 `(event_id, member_id)`로 새 레코드를 다시 적재할 수 있다.
...
- soft delete된 `event_entry`는 현재 유효 출석 이력으로 보지 않으므로, 동일 키로 재출석이 가능하다.
