...
- `event_applicant.round_id`는 필수값이어야 한다.
- 이벤트 생성 시 `event_round`는 최소 1개 생성되므로, `event_applicant`는 기준 회차의 `round_id`를 항상 가져야 한다.
...
- `event_applicant.round_id`를 `NOT NULL`로 유지
