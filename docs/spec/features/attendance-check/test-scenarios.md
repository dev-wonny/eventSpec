...
| ID | 시나리오 | 기대 결과 | 연결 규칙 |
| --- | --- | --- | --- |
| ATT-TEST-001 | 보상 매핑이 있는 회차의 첫 출석 요청 | eligibility 검증 후 외부 point API 성공 시 `event_entry`, `event_win`이 함께 저장된다 | ATT-RULE-002, ATT-RULE-003, ATT-RULE-004, ATT-RULE-009, ATT-RULE-012 |
| ATT-TEST-002 | 보상 매핑이 있는 다른 날짜 회차 재출석 요청 | 같은 `event_applicant` eligibility를 기준으로 외부 point API 성공 후 `event_entry`, `event_win`이 추가 저장된다 | ATT-RULE-001, ATT-RULE-003, ATT-RULE-004, ATT-RULE-009, ATT-RULE-012 |
...
| ATT-TEST-007 | 동일 `event_id + round_id + member_id` 조건의 동시 출석 요청 2건 이상 발생 | 최종 유효 출석은 한 건만 남고, 나머지는 이미 출석 오류로 정리된다 | ATT-RULE-005, ATT-RULE-007 |
...
- 실제 에러 코드와 메시지 매핑
- DDL에 `event_applicant (event_id, member_id)` unique와 `event_entry.event_id`, `event_entry.round_id`, `(event_id, round_id, member_id)` 조회 index가 정확히 반영되는지 테스트
